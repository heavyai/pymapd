...
class TestLoaders:
    ...
    def test_build_table_columnar_pandas(self):
        ...
        result = _pandas_loaders.build_input_columnar(data, preserve_index=False)

        nulls = [False, False]
        expected = [
            TColumn(TColumnData(int_col=[True, False]), nulls=nulls),
            TColumn(
                TColumnData(int_col=np.array([0, 1], dtype=np.int16)), nulls=nulls
            ),  # noqa
            TColumn(
                TColumnData(int_col=np.array([0, 1], dtype=np.int32)), nulls=nulls
            ),  # noqa
            TColumn(
                TColumnData(int_col=np.array([0, 1], dtype=np.int64)), nulls=nulls
            ),  # noqa
            TColumn(
                TColumnData(real_col=np.array([0, 1], dtype=np.float32)), nulls=nulls
            ),  # noqa
            TColumn(
                TColumnData(real_col=np.array([0, 1], dtype=np.float64)), nulls=nulls
            ),  # noqa
    ...
    def test_build_table_columnar_nulls(self):
        ...
        data = pd.DataFrame(
            {
                ...
                "bigint_": np.array([0, 9223372036854775807, None], dtype=np.object),
                ...
                "date_": [datetime.date(1001, 1, 1), datetime.date(2017, 1, 1), None],
            },
            columns=[
                "boolean_",
                "int_",
        ...
        expected = [
            ...
            TColumn(
                TColumnData(real_col=np.array([0, 1, double_na], dtype=np.float64)),
                nulls=nulls,
            ),  # noqa
            TColumn(TColumnData(str_col=["a", "b", ""]), nulls=nulls),
            TColumn(TColumnData(str_col=["a", "b", ""]), nulls=nulls),
            ...
            TColumn(
                TColumnData(int_col=[1451606400, 1483228800, ns_na]), nulls=nulls
            ),  # noqa
            TColumn(
                TColumnData(int_col=[-30578688000, 1483228800, bigint_na]), nulls=nulls
            ),  # noqa
        ]
        assert_columnar_equal(result[0], expected)
    ...
    def test_build_row_desc(self):
        ...
        expected = [
            ...
            TColumnType(
                col_name="int_", col_type=TTypeInfo(type=1), is_reserved_keyword=None
            ),
            ...
            TColumnType(col_name="varchar_", col_type=TTypeInfo(type=6, encoding=4)),
            TColumnType(col_name="text_", col_type=TTypeInfo(type=6, encoding=4)),
        ...
        expected.insert(0, TColumnType(col_name="idx", col_type=TTypeInfo(type=2)))
