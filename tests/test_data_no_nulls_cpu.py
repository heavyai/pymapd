...
class TestCPUDataNoNulls:
    ...
    def test_create_load_table_no_nulls_sql_execute(self, con, method):
        ...
        df_in.drop(columns=["point_", "line_", "mpoly_", "poly_"], inplace=True)
        ...
        df_in.sort_values(by=["tinyint_", "smallint_", "int_", "bigint_"], inplace=True)
        ...
        assert pd.DataFrame.equals(df_in["tinyint_"], df_out["tinyint_"].astype("int8"))

        assert pd.DataFrame.equals(
            df_in["smallint_"], df_out["smallint_"].astype("int16")
        )

        assert pd.DataFrame.equals(df_in["int_"], df_out["int_"].astype("int32"))
    ...
    def test_load_table_geospatial_no_nulls(self, con, method):
        ...
        assert all([x.equals_exact(y, 0.000001) for x, y in zip(point_in, point_out)])

        line_in = [wkt.loads(x) for x in df_in["line_"]]
        line_out = [wkt.loads(x) for x in df_out["line_"]]
        assert all([x.equals_exact(y, 0.000001) for x, y in zip(line_in, line_out)])

        mpoly_in = [wkt.loads(x) for x in df_in["mpoly_"]]
        mpoly_out = [wkt.loads(x) for x in df_out["mpoly_"]]
        assert all([x.equals_exact(y, 0.000001) for x, y in zip(mpoly_in, mpoly_out)])
        ...
        assert all([x.equals_exact(y, 0.011) for x, y in zip(poly_in, poly_out)])
